    ...
    <ScrollView style={[styles.container]}>
      ...
      <View style={styles.card}>
        <View style={styles.cardHeader}>
          <MaterialCommunityIcons name="chart-timeline-variant" size={24} />
        ...
        <View style={styles.section}>
          {Object.entries(insights.categories).map(([category, count]) => (
            <Text key={category} style={[styles.text]}>
              {i18n.t(`INSIGHTS.CATEGORY_COUNT.${category.toUpperCase()}`, { count })}
            </Text>
          ))}
        </View>
      </View>
      ...
      <View style={styles.card}>
        ...
        <View style={styles.section}>
          ...
          <Text style={[styles.text]}>
            {i18n.t('INSIGHTS.AVG_SLEEP', { hours: insights.trends.averageSleepHours.toFixed(1) || '0' })}
          </Text>
          <Text style={[styles.text]}>
      ...
      {(insights.trends.weightTrend || insights.trends.bloodPressureTrend) && (
        <View style={styles.card}>
          <View style={styles.cardHeader}>
            <MaterialCommunityIcons name="heart-pulse" size={24} />
            <Text style={[styles.cardHeader]}>
              {i18n.t('INSIGHTS.HEALTH_METRICS')}
            </Text>
          </View>
          <View style={styles.section}>
            {insights.trends.weightTrend && (
              <>
                <Text style={[styles.text]}>
                  {`Current weight: ${insights.trends.weightTrend.current} kg`}
                </Text>
                <Text style={[styles.text]}>
                  {`Weight change over ${insights.trends.weightTrend.period}: ${
                    insights.trends.weightTrend.change > 0 ? '+' : ''
                  }${insights.trends.weightTrend.change.toFixed(1)} kg`}
                </Text>
              </>
            )}
            ...
            {insights.trends.bloodPressureTrend && (
              <>
                <Text style={[styles.text]}>
                  {`Average blood pressure: ${Math.round(insights.trends.bloodPressureTrend.systolic.avg)}/${
                    Math.round(insights.trends.bloodPressureTrend.diastolic.avg)
                  } mmHg`}
                </Text>
                <Text style={[styles.text]}>
                  {`Blood pressure change over ${insights.trends.bloodPressureTrend.period}:`}
                </Text>
                <Text style={[styles.text]}>
                  {`  Systolic: ${
                    insights.trends.bloodPressureTrend.systolic.change > 0 ? '+' : ''
                  }${Math.round(insights.trends.bloodPressureTrend.systolic.change)} mmHg`}
                </Text>
                <Text style={[styles.text]}>
                  {`  Diastolic: ${
                    insights.trends.bloodPressureTrend.diastolic.change > 0 ? '+' : ''
                  }${Math.round(insights.trends.bloodPressureTrend.diastolic.change)} mmHg`}
                </Text>
              </>
            )}
          </View>
      ...
      <View style={styles.card}>
        <View style={styles.cardHeader}>
          <MaterialCommunityIcons name="connection" size={24} />
          <Text style={[styles.cardHeader]}>
            {i18n.t('INSIGHTS.CATEGORIES')}
          </Text>
        </View>
        <View style={styles.section}>
          ...
          {insights.correlations.nutritionSymptoms.length > 0 && (
            <>
              <Text style={[styles.cardHeader]}>
                {i18n.t('INSIGHTS.MOST_TRACKED_SYMPTOM', { symptom: "Foods that might affect you" })}
              </Text>
              {insights.correlations.nutritionSymptoms.map((correlation, index) => (
                <Text key={`nutrition-${index}`} style={[styles.text]}>
                  {`When you eat ${correlation.food}, ${correlation.symptom} appears ${correlation.frequency} times`}
                </Text>
              ))}
            </>
          )}
          ...
          {insights.correlations.sleepNutrition.length > 0 && (
            <>
              <Text style={[styles.cardHeader]}>
                {i18n.t('INSIGHTS.AVG_SLEEP')}
              </Text>
              {insights.correlations.sleepNutrition.map((correlation, index) => (
                <Text key={`sleep-${index}`} style={[styles.text]}>
                  {`On days when you eat ${correlation.food}, your sleep quality is ${(correlation.avgSleepQuality * 100).toFixed(0)}%`}
                </Text>
              ))}
            </>
          )}
          ...
          {insights.correlations.cycleSymptoms.length > 0 && (
            <>
              <Text style={[styles.cardHeader]}>
                {i18n.t('INSIGHTS.MOST_TRACKED_SYMPTOM', { symptom: "Symptoms during your cycle" })}
              </Text>
              {insights.correlations.cycleSymptoms.map((correlation, index) => (
                <Text key={`cycle-${index}`} style={[styles.text]}>
                  {`During your cycle, ${correlation.symptom} appears ${correlation.frequency} times`}
                </Text>
              ))}
            </>
          )}
          ...
          {insights.correlations.moodSymptoms.length > 0 && (
            <>
              <Text style={[styles.cardHeader]}>
                {i18n.t('INSIGHTS.COMMON_MOOD', { pattern: "How your mood relates to symptoms" })}
              </Text>
              {insights.correlations.moodSymptoms.map((correlation, index) => (
                <Text key={`mood-${index}`} style={[styles.text]}>
                  {`When you feel ${correlation.mood}, ${correlation.symptom} appears ${correlation.frequency} times`}
                </Text>
              ))}
            </>
          )}
            ...
            <>
              ...
              {insights.correlations.activitySleep.map((correlation, index) => (
                <Text key={`activity-sleep-${index}`} style={[styles.text]}>
                  {`On days when you do ${correlation.activity}, your sleep quality is ${(correlation.avgSleepQuality * 100).toFixed(0)}%`}
                </Text>
              ))}
            </>
          )}
          ...
          {insights.correlations.medicationEffectiveness.length > 0 && (
            <>
              <Text style={[styles.cardHeader]}>
                {i18n.t('INSIGHTS.MED_ADHERENCE')}
              </Text>
              {insights.correlations.medicationEffectiveness.map((correlation, index) => (
                <Text key={`medication-${index}`} style={[styles.text]}>
                  {`When you take ${correlation.medication}, symptoms reduce by ${(correlation.symptomReduction * 100).toFixed(0)}%`}
                </Text>
              ))}
            </>
          )}
          ...
          {insights.correlations.activityMood.length > 0 && (
            <>
              <Text style={[styles.cardHeader]}>
                {i18n.t('INSIGHTS.COMMON_MOOD', { pattern: "Activities and your mood" })}
              </Text>
              {insights.correlations.activityMood.map((correlation, index) => (
                <Text key={`activity-mood-${index}`} style={[styles.text]}>
                  {`When you do ${correlation.activity}, you often feel ${correlation.mood} (${correlation.frequency} times)`}
                </Text>
              ))}
            </>
          )}
          ...
          {insights.correlations.healthSymptoms?.length > 0 && (
            <>
              <Text style={[styles.cardHeader]}>
                {i18n.t('INSIGHTS.MOST_TRACKED_SYMPTOM', { symptom: i18n.t('INSIGHTS.CATEGORIES') })}
              </Text>
              {insights.correlations.healthSymptoms.map((correlation, index) => (
                <Text key={`health-symptom-${index}`} style={[styles.text]}>
                  {`When you have ${correlation.health}, ${correlation.symptom} appears ${correlation.frequency} times`}
                </Text>
              ))}
            </>
          )}
          ...
          {insights.correlations.healthMood?.length > 0 && (
            <>
              <Text style={[styles.cardHeader]}>
                {i18n.t('INSIGHTS.COMMON_MOOD', { pattern: i18n.t('INSIGHTS.CATEGORIES') })}
              </Text>
              {insights.correlations.healthMood.map((correlation, index) => (
                <Text key={`health-mood-${index}`} style={[styles.text]}>
                  {`When you have ${correlation.health}, you often feel ${correlation.mood} (${correlation.frequency} times)`}
                </Text>
              ))}
            </>
          )}
